...
<div class="form-group col-sm-6">
    ...
    <input type="date" id="maindate" name="maindate" class="form-control" value="{{$invoice->date}}">
</div>
...
<div class="form-group col-sm-6">
    ...
    <input type="text" id="current_stock_qty" class="form-control form-control-sm" readonly style="background-color: #D8FDBA">
</div>
    ...
    <form method="post" action="" id="myForm">
        ...
        <table class="table-sm table-bordered" width="100%" style="align:center">
            ...
            <tbody id="addRow" class="addRow">
                ...
                <tr class="delete_add_more_item" id="delete_add_more_item">
                    ...
                    <input type="hidden" name="invoice_no" value="{{$detail->invoice_id}}">
                    <input type="hidden" id="invoice_id" name="invoice_id[]" value="{{$detail->id}}">
                    <td>
                    ...
                    <td>
                        <input type="number" min="1" class="form-control form-control-sm text-right selling_qty" name="selling_qty[]" value="{{$detail->selling_qty}}">
                    </td>
                    <td>
                        <input type="number" min="1" class="form-control form-control-sm text-right unit_price" name="unit_price[]" value="{{$detail->unit_price}}">
                    </td>

                    <td> <input class="form-control form-control-sm text-right selling_price" name="selling_price[]" value="{{$detail->selling_price}}">
                    </td>
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <input type="text" name="discount_amount" id="discount_amount" class="form-control form-control-sm text-right discount_amount" value="{{$invoice->payment->discount_amount}}">
                    </td>
                </tr>
                <tr>
        ...
        <div class="form-row">
            <div class="form-group col-sm-12">
                <textarea name="description" class="form-control" id="description" placeholder="White desciption here">{{$invoice->description}}</textarea>
            </div>
        </div>
        <div class="form-row">
            <div class="form-group col-sm-3">
                ...
                <select name="paid_status" id="paid_status" class="form-control form-control-sm">
                    ...
                    <option value="full_paid" @if($invoice->payment->paid_status == 'full_paid') selected @endif>Full Paid</option>
                    <option value="full_due" @if($invoice->payment->paid_status == 'full_due') selected @endif>Full Due</option>
                    <option value="partial_paid" @if($invoice->payment->paid_status == 'partial_paid') selected @endif>Partial Paid</option>
                </select>
                <input type="text" name="patial_paid_amount" class="form-control form-control-sm paid_status" value="{{$invoice->payment->paid_amount}}" style="display:none;">
            </div>
            <div class="form-group col-sm-9">
                ...
                <select name="customer_id" id="customer_name" class="form-control form-control-sm">
                    ...
                    <option value="{{$customer->id}}" @if($customer->id == $invoice->payment->customer_id) selected @endif>{{$customer->name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group col-sm-3">
            <label></label>
            <button type="submit" class="btn btn-primary" id="storeButton" style="clear:both;">Update Purchase</button>
        </div>
    </form>
</div>
